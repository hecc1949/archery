...
{
    setAttribute(Qt::WA_DeleteOnClose, false);      //my
    ...
    QPalette palette;
    palette.setBrush(QPalette::Background, QPixmap(":/style/res/background.jpg"));
...
int SoftKeyBoardDlg::startExec(QString *value, SKB_Page page)
{
    softKeyValue = value;
    if (page == SKB_Page_Digit)
        ui->stackedWidget->setCurrentWidget(ui->page_digit);
    else if (page == SKB_Page_ExtChar)
        ui->stackedWidget->setCurrentWidget(ui->page_chars);
    else
    {
        ui->stackedWidget->setCurrentWidget(ui->page_spell);
        if (page ==SKB_Page_EnglishChar)
        {
            bIsEnglishMode = true;
            ui->pushButton_change->setText(enLanguage);
        }
